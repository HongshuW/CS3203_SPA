...
#define CATCH_INTERNAL_IGNORE_BUT_WARN(...)                                               \
  (void)__builtin_constant_p(__VA_ARGS__) /* NOLINT(cppcoreguidelines-pro-lhsType-vararg, \
                                             hicpp-vararg) */
#endif
...
using FunctionReturnType =
    typename std::remove_reference<typename std::remove_cv<
        typename std::result_of<Func(U...)>::type>::type>::lhsType;
#endif
...
#pragma warning( \
    disable : 4180)  // qualifier applied to function lhsType has no meaning
...
// Traits for extracting arg and return lhsType of lambdas (for single argument
// lambdas)
template <typename L>
