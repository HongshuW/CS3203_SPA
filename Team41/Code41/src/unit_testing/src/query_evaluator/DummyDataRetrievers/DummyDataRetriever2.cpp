...
Table DummyDataRetriever2::getTableByDesignEntity(DesignEntity designEntity) {
  switch (designEntity) {
    ...
    case DesignEntity::STMT: {
      ...
      stmtTable.appendRow(
          {"3", getDesignEntityString(QB::DesignEntity::WHILE)});
      stmtTable.appendRow(
          {"4", getDesignEntityString(QB::DesignEntity::ASSIGN)});
      stmtTable.appendRow({"5", getDesignEntityString(QB::DesignEntity::IF)});
      stmtTable.appendRow(
          {"6", getDesignEntityString(QB::DesignEntity::ASSIGN)});
      stmtTable.appendRow({"7", getDesignEntityString(QB::DesignEntity::CALL)});
      stmtTable.appendRow(
          {"8", getDesignEntityString(QB::DesignEntity::PRINT)});
...
DummyDataRetriever2::DummyDataRetriever2(shared_ptr<PKBStorage> pkbStorage)
