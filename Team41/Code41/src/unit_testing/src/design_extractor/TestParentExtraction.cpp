...
TEST_CASE("Test Parent and Parent* Extraction") {
  SECTION("procedure 1") {
    ...
    // Test Parent
    shared_ptr<list<vector<string>>> actualParentRelations =
        ParentExtractor::extractParent(programNode);
    ...
    // Test Parent*
    shared_ptr<list<vector<string>>> actualParentTRelations =
        ParentExtractor::extractParentT(programNode);
  ...
  SECTION("procedure 3") {
    ...
    shared_ptr<list<vector<string>>> actualParentTRelations =
        ParentExtractor::extractParentT(programNode);
    // Parent and Parent* should return the same results, since there are only 2
    // nesting levels
