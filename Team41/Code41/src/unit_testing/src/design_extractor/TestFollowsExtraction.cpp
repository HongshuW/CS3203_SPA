...
TEST_CASE("Test Follows Extractor") {
    ...
    SECTION("test non-nested + singly-nested procedures") {
        ...
        DesignExtractor designExtractor = DesignExtractor(dataModifier, programNode);
        auto actual = designExtractor.extractRelations(QB::RelationType::FOLLOWS);
        vector<vector<string>> expected = { {"1", "2"}, {"2", "3"}, {"4", "5"}, {"5", "6"},
                                            {"6", "10"}, {"7", "8"}};
        std::list<vector<string>>::iterator it;
        REQUIRE(expected.size() == actual->size());
        REQUIRE(TestDE::DEUtils::containsSameElementPair(*actual, expected));
    ...
    SECTION("test doubly-nested + doubly-nested procedures") {
        ...
        DesignExtractor designExtractor = DesignExtractor(dataModifier, programNode);
        auto actual = designExtractor.extractRelations(QB::RelationType::FOLLOWS);
        vector<vector<string>> expected = { {"1", "2"}, {"2", "3"}, {"3", "9"}, {"4", "5"},
                                            {"6", "7"}, {"10", "11"}, {"11", "16"}, {"12", "13"}};
        std::list<vector<string>>::iterator it;
        REQUIRE(expected.size() == actual->size());
        REQUIRE(TestDE::DEUtils::containsSameElementPair(*actual, expected));
    }
