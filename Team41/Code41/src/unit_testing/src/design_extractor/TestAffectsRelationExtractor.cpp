...
*/
  ...
  SECTION("Test Both Invalid Arguments") {
      ...
      AffectsRelationExtractor affectsRelationExtractor = AffectsRelationExtractor(dataModifier, programNode);
  ...
  SECTION("Test Both Valid Arguments But Invalid Answer In If Procedure") {
      ...
      args.setEndStmtNo(1);
      vector<string> actual = *static_pointer_cast<QueryTimeResult>(affectsRelationExtractor.extract(args))->getResult();
      vector<string> expected = {};
      REQUIRE(actual == expected);
  }
