...
TEST_CASE("Test AST utils") {
  SECTION("Test line number map traversal with no stmt node") {
    ...
    shared_ptr<unordered_map<shared_ptr<ASTNode>, int>> actual_nodeToLine =
        make_shared<unordered_map<shared_ptr<ASTNode>, int>>();
    shared_ptr<unordered_map<int, shared_ptr<ASTNode>>> actual_lineToNode =
        make_shared<unordered_map<int, shared_ptr<ASTNode>>>();
    ASTUtils::getNodeLineMapping(programNode, actual_nodeToLine,
                                 actual_lineToNode);

    ASTUtils::getNodeLineMapping(programNode, actual_nodeToLine,
                                 actual_lineToNode);

    unordered_map<shared_ptr<ASTNode>, int> expected_nodeToLine =
        unordered_map<shared_ptr<ASTNode>, int>{};
    REQUIRE(*actual_nodeToLine == expected_nodeToLine);

    unordered_map<int, shared_ptr<ASTNode>> expected_lineToNode =
        unordered_map<int, shared_ptr<ASTNode>>{};
  ...
  SECTION("Test line number map traversal without nesting") {
    ...
    shared_ptr<ProcedureNode> procedureNode = make_shared<ProcedureNode>(
        ProcedureNode("procedure1", {printNode, readNode}));
    shared_ptr<ProgramNode> programNode =
        make_shared<ProgramNode>(ProgramNode({procedureNode}));
    shared_ptr<unordered_map<shared_ptr<ASTNode>, int>> actual_nodeToLine =
        make_shared<unordered_map<shared_ptr<ASTNode>, int>>();
    ...
    ASTUtils::getNodeLineMapping(programNode, actual_nodeToLine,
                                 actual_lineToNode);
    unordered_map<shared_ptr<ASTNode>, int> expected_nodeToLine =
        unordered_map<shared_ptr<ASTNode>, int>{{printNode, 1}, {readNode, 2}};

    REQUIRE(*actual_nodeToLine == expected_nodeToLine);

    unordered_map<int, shared_ptr<ASTNode>> expected_lineToNode =
        unordered_map<int, shared_ptr<ASTNode>>{{1, printNode}, {2, readNode}};
    REQUIRE(expected_lineToNode == *actual_lineToNode);
  }
  SECTION("Test line number map traversal with single nesting") {
    ...
    shared_ptr<StmtNode> ifNode =
        make_shared<IfNode>(condExpr, ifStmtList, elseStmtList);
    ...
    unordered_map<shared_ptr<ASTNode>, int> expected_nodeToLine =
        unordered_map<shared_ptr<ASTNode>, int>{{printNode, 1}, {readNode, 2},
                                                {ifNode, 3},    {printNode2, 4},
                                                {readNode2, 5}, {readNode3, 6}};
    REQUIRE(*actual_nodeToLine == expected_nodeToLine);

    unordered_map<int, shared_ptr<ASTNode>> expected_lineToNode =
        unordered_map<int, shared_ptr<ASTNode>>{{1, printNode}, {2, readNode},
                                                {3, ifNode},    {4, printNode2},
                                                {5, readNode2}, {6, readNode3}};
    REQUIRE(expected_lineToNode == *actual_lineToNode);
  }
  SECTION("Test line number map traversal with double nesting") {
    ...
    shared_ptr<StmtNode> whileNode =
        make_shared<WhileNode>(condExpr, whileStmtList);
    ...
    unordered_map<shared_ptr<ASTNode>, int> expected_nodeToLine =
        unordered_map<shared_ptr<ASTNode>, int>{
            {printNode, 1},  {readNode, 2},  {ifNode, 3},
            {printNode2, 4}, {whileNode, 5}, {readNode4, 6},
            {printNode4, 7}, {readNode2, 8}, {readNode3, 9}};
    REQUIRE(*actual_nodeToLine == expected_nodeToLine);

    unordered_map<int, shared_ptr<ASTNode>> expected_lineToNode =
        unordered_map<int, shared_ptr<ASTNode>>{
            {1, printNode},  {2, readNode},  {3, ifNode},
            {4, printNode2}, {5, whileNode}, {6, readNode4},
            {7, printNode4}, {8, readNode2}, {9, readNode3}};
    REQUIRE(expected_lineToNode == *actual_lineToNode);
  }
  SECTION("Test two procedure") {
     ...
     * }
     */
    shared_ptr<StmtNode> printNode_p1 =
        make_shared<PrintNode>(make_shared<VariableNode>("x"));
    shared_ptr<StmtNode> readNode_p1 =
        make_shared<ReadNode>(make_shared<VariableNode>("y"));
    shared_ptr<ProcedureNode> procedureNode_p1 = make_shared<ProcedureNode>(
        ProcedureNode("procedure1", {printNode_p1, readNode_p1}));
    ...
    shared_ptr<ProgramNode> programNode = make_shared<ProgramNode>(
        ProgramNode({procedureNode_p1, procedureNode2}));
    shared_ptr<unordered_map<shared_ptr<ASTNode>, int>> actual_nodeToLine =
        make_shared<unordered_map<shared_ptr<ASTNode>, int>>();
    shared_ptr<unordered_map<int, shared_ptr<ASTNode>>> actual_lineToNode =
        make_shared<unordered_map<int, shared_ptr<ASTNode>>>();
    ...
    unordered_map<shared_ptr<ASTNode>, int> expected_nodeToLine =
        unordered_map<shared_ptr<ASTNode>, int>{
            {printNode_p1, 1}, {readNode_p1, 2}, {printNode, 3},
            {readNode, 4},     {ifNode, 5},      {printNode2, 6},
            {readNode2, 7},    {readNode3, 8}};
    REQUIRE(*actual_nodeToLine == expected_nodeToLine);

    unordered_map<int, shared_ptr<ASTNode>> expected_lineToNode =
        unordered_map<int, shared_ptr<ASTNode>>{
            {1, printNode_p1}, {2, readNode_p1}, {3, printNode},
            {4, readNode},     {5, ifNode},      {6, printNode2},
            {7, readNode2},    {8, readNode3}};
  }
}