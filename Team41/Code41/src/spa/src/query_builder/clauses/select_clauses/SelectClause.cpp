...
SelectClause::SelectClause(ReturnType returnType,
...
bool SelectClause::operator==(const SelectClause& clause) const {
  ...
  bool isEqual =
      selectClause != nullptr && returnType == selectClause->returnType;
  if (returnResults) {
    isEqual =
        isEqual && equal(begin(*returnResults), end(*returnResults),
                         begin(*selectClause->returnResults),
                         end(*selectClause->returnResults),
                         [](const Elem& l, const Elem& o) { return l == o; });
  }
  return isEqual;
}
