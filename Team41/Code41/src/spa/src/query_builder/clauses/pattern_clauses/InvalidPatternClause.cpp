...
int InvalidPatternClause::validateExprSpec(int currIdx, const vector<string> &tokens) {
    vector<string> expr;
    if (tokens[currIdx] == QueryParserConstants::DOUBLE_QUOTE) {
        ...
        if (exprNode == nullptr) {
            string errorMessage = ErrorMessageFormatter::formatErrorMessage(
                    QueryParserConstants::PQL_PARSE_EXCEPTION_EXPECT_EXPRESSION, tokens[currIdx]);
