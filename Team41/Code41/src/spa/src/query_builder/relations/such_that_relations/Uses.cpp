...
Uses::Uses(shared_ptr<vector<Declaration>> declarations) : declarations(declarations) {};
...
shared_ptr<AbstractClause> Uses::createClause(Ref arg1, Ref arg2) {
    if (getRefType(arg1) == RefType::INTEGER) {
        ...
        if (auto declaration = Declaration::findDeclaration(*synonym, declarations)) {
            ...
            // Arg1 is not declared, return UsesS as default, semantic error will be thrown in validator
            return make_shared<UsesSClause>(arg1, arg2);
        }
    } else {
