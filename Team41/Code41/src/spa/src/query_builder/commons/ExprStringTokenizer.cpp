...
ExprStringTokenizer::ExprStringTokenizer(string exprStr)
    : currIdx(0), exprStr(std::move(exprStr)) {}
...
vector<string> ExprStringTokenizer::tokenize() {
  ...
  for (int i = 0; i < exprTokens.size(); i++) {
    ...
    //! The first and the last string in the exprTokens cannot be a math
    //! operator
    if (Utils::isMathOperators(exprTokens[i]) && isNotFirstAndLastChar(i))
      continue;
  ...
  if (!Utils::isValidParentheses(exprTokens)) {
    throw PQLParseException(
        QueryTokenizerConstants::QPL_EXPR_STRING_INVALID_PARENTHESES);
  }
