...
bool Cachable::areAllRelationsCached() {
    return allRelationsCached;
}
...
/**
 * Given a query, check whether it has been cached, if so, return the range in the cache table.
 *
 ...
 *
 * @param query vector<string>{firstStatement, secondStatement, startIndex, endIndex}
 */
void Cachable::setQueried(vector<string> query) {
    cacheInformation.push_back(query);
}

void Cachable::clearCacheInformation() {
