...
Tokenizer::Tokenizer(std::string source)
    : currIdx(TokenizerConstants::ZERO), source(std::move(source)) {}

bool Tokenizer::isWithinBound() { return currIdx < source.length(); }
...
bool Tokenizer::match(char c) { return peek() == c; }
...
void Tokenizer::processSymbols() {
  const unordered_set<string> ALLOWED_TOKENS_FOR_EQUALITY = {
      TokenizerConstants::EQUAL, TokenizerConstants::GREATER_THAN,
      TokenizerConstants::SMALLER_THAN, TokenizerConstants::EXCLAMATION_MARK
  ...
  if (ALLOWED_TOKENS_FOR_EQUALITY.count(curr)) {
    ...
    if (!match(TokenizerConstants::SINGLE_AND_CHAR)) {
      string errorMessage = ErrorMessageFormatter::formatErrorMessage(
          ParserConstants::AND_OP, TokenizerConstants::SINGLE_AND);
    ...
    if (!match(TokenizerConstants::BAR_CHAR)) {
      string errorMessage = ErrorMessageFormatter::formatErrorMessage(
          ParserConstants::OR_OP, TokenizerConstants::BAR);
      throw SPTokenizeException(errorMessage);
    }
    curr += pop();
  }
...
vector<string> Tokenizer::tokenize() {
  ...
  while (isWithinBound()) {
    ...
    if (isspace(next)) {
      ...
      throw SPTokenizeException(TokenizerConstants::UNEXPECTED_TOKEN_MESSAGE +
                                next);
    }
  }
  return tokens;
