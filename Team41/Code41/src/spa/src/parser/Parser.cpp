...
shared_ptr<ProcedureNode> Parser::parseProcedureNode() {
  ...
  if (!Utils::isValidName(procedureName)) {
    throw SPParseException(
        ParserConstants::SP_PARSE_EXCEPTION_INVALID_PROC_NAME + procedureName);
  }
...
shared_ptr<IfNode> Parser::parseIfNode() {
  ...
  shared_ptr<IfNode> ifNode =
      make_shared<IfNode>(condExprNode, ifStatementList, elseStatementList);
  return ifNode;
}
...
shared_ptr<AssignNode> Parser::parseAssignNode() {
  ...
  shared_ptr<AssignNode> assignNode =
      make_shared<AssignNode>(variableNode, exprNode);
  return assignNode;
}
...
shared_ptr<WhileNode> Parser::parseWhileNode() {
  ...
  shared_ptr<WhileNode> whileNode =
      make_shared<WhileNode>(condExprNode, statementList);
  return whileNode;
}
...
shared_ptr<ExprNode> Parser::parseExprNode() {
  ...
  while (peek() != ParserConstants::SEMICOLON) {
    ...
    if (Utils::isBracket(curr) || Utils::isMathOperators(curr) ||
      ...
      //! We need to ensure all tokens are valid before passing to the
      //! ExprNodeParser
      expr.push_back(curr);
    } else {
      throw SPParseException(
          ParserConstants::SP_PARSE_EXCEPTION_INVALID_TOKEN_EXPR);
    }
  }
  if (expr.empty()) {
    throw SPParseException(
        ParserConstants::SP_PARSE_EXCEPTION_ASSIGN_STMT_RHS_EMPTY);
  }
...
int Parser::parseSymbol(const string& curr) {
  const unordered_set<string> ALLOWED_TOKENS = unordered_set<string>(
      {ParserConstants::LESS_THAN, ParserConstants::LESS_THAN_AND_EQUAL_TO,
       ParserConstants::GREATER_THAN,
       ParserConstants::GREATER_THAN_AND_EQUAL_TO, ParserConstants::EQUAL_TO,
       ParserConstants::NOT_EQUAL_TO, ParserConstants::SEMICOLON,
       ParserConstants::RIGHT_BRACKET});

  if (ALLOWED_TOKENS.count(curr) || currIdx >= tokens.size() ||
      curr == ParserConstants::RIGHT_BRACKET) {
    ...
    return ParserConstants::ONE;
             ...
             curr == ParserConstants::DIVIDE ||
             curr == ParserConstants::MODULO) {
...
shared_ptr<ExprNode> Parser::makeExprNode(const string& curr,
...
shared_ptr<RelExprNode> Parser::parseRelExprNode() {
  ...
  if (!valid_ops.count(op)) {
    throw SPParseException(
        ParserConstants::SP_PARSE_EXCEPTION_EXPECT_COMPARATOR_OP);
  }
