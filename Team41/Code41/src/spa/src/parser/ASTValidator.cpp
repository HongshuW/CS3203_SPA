  ...
  for (const auto& procedure : ast->procedureList) {
    ...
    if (iterator != procedureNames.end()) {
      string errorMessage = ErrorMessageFormatter::formatErrorMessage(
          ValidatorConstants::SP_AST_EXCEPTION_DUPLICATE_PROCEDURE,
          procedureName);
...
void ASTValidator::validateCallNode(const shared_ptr<CallNode>& callNode,
                                    const string& procedureName) {
  string calledProcedure = callNode->procedureName;

  if (calledProcedure == procedureName) {
    string errorMessage = ErrorMessageFormatter::formatErrorMessage(
        ValidatorConstants::SP_AST_EXCEPTION_PROCEDURE_CALLS_ITSELF,
        procedureName);
    throw SPValidationException(errorMessage);
  }
  ...
  if (iterator == procedureNames.end()) {
    string errorMessage = ErrorMessageFormatter::formatErrorMessage(
        ValidatorConstants::SP_AST_EXCEPTION_PROCEDURE_NOT_FOUND,
        procedureName);
    throw SPValidationException(errorMessage);
  }
...
void ASTValidator::calls(const string& procedure,
                         const string& calledProcedure) {
  if (procedure == calledProcedure) {
    throw SPValidationException(
        ValidatorConstants::SP_AST_EXCEPTION_CYCLIC_DEPENDENCY);
  }
