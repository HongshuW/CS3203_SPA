...
void PKBSaverExecutor::initRelations(
    vector<shared_ptr<PKBSaverInterface>>& extractedDesignList) {
  auto calls =
      make_shared<ExtractedCallsSaver>(this->programNode, this->dataModifier);
  extractedDesignList.push_back(calls);

  auto follows =
      make_shared<ExtractedFollowsSaver>(this->programNode, this->dataModifier);
  extractedDesignList.push_back(follows);

  auto modifies = make_shared<ExtractedModifiesSaver>(this->programNode,
                                                      this->dataModifier);
  extractedDesignList.push_back(modifies);

  auto next =
      make_shared<ExtractedNextSaver>(this->programNode, this->dataModifier);
  extractedDesignList.push_back(next);

  auto uses =
      make_shared<ExtractedUsesSaver>(this->programNode, this->dataModifier);
  extractedDesignList.push_back(uses);

  auto parents =
      make_shared<ExtractedParentsSaver>(this->programNode, this->dataModifier);
  extractedDesignList.push_back(parents);
}

void PKBSaverExecutor::initPatterns(
    vector<shared_ptr<PKBSaverInterface>>& extractedDesignList) {
  auto assignPattern = make_shared<ExtractedAssignPatternSaver>(
      this->programNode, this->dataModifier);
  extractedDesignList.push_back(assignPattern);

  auto ifPattern = make_shared<ExtractedIfPatternSaver>(this->programNode,
                                                        this->dataModifier);
  extractedDesignList.push_back(ifPattern);

  auto whilePattern = make_shared<ExtractedWhilePatternSaver>(
      this->programNode, this->dataModifier);
  extractedDesignList.push_back(whilePattern);
}

void PKBSaverExecutor::initEntities(
    vector<shared_ptr<PKBSaverInterface>>& extractedDesignList) {
  auto variables = make_shared<ExtractedVariableSaver>(this->programNode,
                                                       this->dataModifier);
  extractedDesignList.push_back(variables);

  auto constants = make_shared<ExtractedConstantSaver>(this->programNode,
                                                       this->dataModifier);
  extractedDesignList.push_back(constants);

  auto procedures = make_shared<ExtractedProceduresSaver>(this->programNode,
                                                          this->dataModifier);
