...
namespace DE {
shared_ptr<list<vector<string>>> UsesExtractor::extractUsesS(
...
unordered_set<string> UsesExtractor::extractUsesSHelper(
  ...
  switch (nodeType) {
    ...
    case AST::IF_NODE: {
      ...
      auto variables =
          EntityExtractor::getVariablesFromCondExprNode(ifNode->condExpr);
    ...
    case AST::WHILE_NODE: {
      ...
      auto variables =
          EntityExtractor::getVariablesFromCondExprNode(whileNode->condExpr);
    ...
    case AST::ASSIGN_NODE: {
      ...
      auto variables =
          EntityExtractor::getVariablesFromExprNode(assignNode->expressionNode);
...
void UsesExtractor::insertCallsForUseS(shared_ptr<ProgramNode> rootPtr,
  ...
  for (auto pair : mappedCallNodesToProcedures) {
    ...
    for (auto callNode : listOfCallNodes) {
      ...
      CallsExtractor::extractCallStmtRelationshipsToOutput(
          stmtNo, callNode, mappedProceduresToUsedVar,
...
shared_ptr<list<vector<string>>> UsesExtractor::extractUsesP(
    shared_ptr<ProgramNode> rootPtr) {
  shared_ptr<list<vector<string>>> ans = make_shared<list<vector<string>>>();
  auto mappedProceduresToUsedVar = mapProceduresToUsedVariables(rootPtr);
  auto mappedCallNodesToProcedures =
    ...
    if (mappedCallNodesToProcedures.count(procedureName) != 0) {
      ...
      for (auto node : callNodes) {
        EntityExtractor::extractVariablesFromCallNodesInProceduresToList(
            node, mappedProceduresToUsedVar, mappedCallNodesToProcedures,
            uniqueVarList);
      }
    }
...
unordered_map<string, unordered_set<string>>
UsesExtractor::mapIfAndWhileStmtNoToUsedVariables(
    shared_ptr<ProgramNode> rootPtr) {
  shared_ptr<unordered_map<shared_ptr<StmtNode>, int>> stmtNumbers =
      ASTUtils::getNodePtrToLineNumMap(rootPtr);
  auto map = unordered_map<string, unordered_set<string>>();
    ...
    while (!queue.empty()) {
      ...
      switch (nodeType) {
        ...
        case AST::WHILE_NODE: {
          ...
          auto variables = EntityExtractor::getVariablesFromCondExprNode(
              whileNode->condExpr);
          uniqueVarList.insert(variables.begin(), variables.end());
        ...
        case AST::ASSIGN_NODE: {
          ...
          auto variables = EntityExtractor::getVariablesFromExprNode(
...
void UsesExtractor::insertCallsInIfAndWhileForUseS(
