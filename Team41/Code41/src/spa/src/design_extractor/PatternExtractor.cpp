...
void PatternExtractor::extractPatternHelper(
...
list<vector<string>> PatternExtractor::extractIfPattern(
...
list<vector<string>> PatternExtractor::extractWhilePattern(
    shared_ptr<ProgramNode> programNode) {
  list<vector<string>> output;
  auto stmtNumbers = ASTUtils::getNodePtrToLineNumMap(programNode);
  for (auto procedureNode : programNode->procedureList) {
...
unordered_set<string> PatternExtractor::condExprNodeHandler(
  ...
  if (condExpr->condExprLHS) {
    PatternExtractor::getRelExprNodesDFS(condExpr->condExprLHS,
                                         relExprNodeList);
  }

  if (condExpr->condExprRHS) {
    PatternExtractor::getRelExprNodesDFS(condExpr->condExprRHS,
                                         relExprNodeList);
  }
...
void PatternExtractor::exprNodeHandler(shared_ptr<ExprNode> expr,
                                       unordered_set<string>& varList) {
  getVarFromExprNodesDFS(expr, varList);
}

void PatternExtractor::getVarFromExprNodesDFS(shared_ptr<ExprNode> expr,
                                              unordered_set<string>& varList) {
...
void PatternExtractor::getRelExprNodesDFS(
    shared_ptr<CondExprNode> condExpr,
    vector<shared_ptr<RelExprNode>>& relExprNodeList) {
  if (condExpr->relExprNode) {
    relExprNodeList.push_back(condExpr->relExprNode);
