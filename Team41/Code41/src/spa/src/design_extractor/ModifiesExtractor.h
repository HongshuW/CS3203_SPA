...
class ModifiesExtractor {
 private:
  static void extractModifiesSDFS(
  ...
  static unordered_set<string> getModifiedVariablesFromProcedure(
      shared_ptr<ProcedureNode> procedureNode);
  static unordered_map<string, unordered_set<string>>
  mapIfAndWhileStmtNoToModifiedVariables(shared_ptr<ProgramNode> rootPtr);
  static void insertCallsInIfAndWhileForModifiesS(
 ...
 public:
  ...
  static shared_ptr<list<vector<string>>> extractModifiesS(
      shared_ptr<ProgramNode> rootPtr);
  static shared_ptr<list<vector<string>>> extractModifiesP(
      shared_ptr<ProgramNode> rootPtr);
};
}  // namespace DE
