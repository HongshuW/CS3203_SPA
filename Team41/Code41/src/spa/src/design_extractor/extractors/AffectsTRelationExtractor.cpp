...
namespace DE {
    AffectsTRelationExtractor::AffectsTRelationExtractor(shared_ptr<DataModifier> dataModifier,
                                                         shared_ptr<ProgramNode> programNode) : AffectsCommonExtractor(
    ...
    shared_ptr<ExtractorResult> AffectsTRelationExtractor::extract(StmtNoArgs args) {
        if (args.startAndEndExists()) {
            if (affectsAdjListPtr == nullptr) initAffectsAdjList();
            return make_shared<QueryTimeResult>(extractNoWildcard(args.getStartStmtNo(), args.getEndStmtNo()));
        }
        if (args.startExistsOnly()) {
            if (affectsAdjListPtr == nullptr) initAffectsAdjList();
            return make_shared<QueryTimeResult>(extractWithStartGiven(args.getStartStmtNo()));
        }
        if (args.endExistsOnly()) {
            if (affectsAdjListReversedPtr == nullptr) initAffectsAdjListReversed();
            return make_shared<QueryTimeResult>(extractWithEndGiven(args.getEndStmtNo()));
        }
    ...
    shared_ptr<ExtractorResult> AffectsTRelationExtractor::extractAllRelations() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (!dp[i][j]) continue;
                output->push_back({to_string(i + EDGE_INDEX_OFFSET), to_string(j + EDGE_INDEX_OFFSET)});
            }
        }
    ...
    shared_ptr<vector<string>> AffectsTRelationExtractor::extractNoWildcard(int start, int end) {
        bool isValid = extractNoWildcardDFS(start, end, make_shared<unordered_set<int>>());
    ...
    bool AffectsTRelationExtractor::extractNoWildcardDFS(int curr, int target,  shared_ptr<unordered_set<int>> visited) {
        visited->insert(curr);
        bool isFound = false;
    ...
    shared_ptr<vector<string>> AffectsTRelationExtractor::extractWithStartGiven(int start) {
        shared_ptr<vector<string>> result = make_shared<vector<string>>();
        extractWithStartGivenDFS(start, start, result, make_shared<unordered_set<int>>());
        return result;
    }

    void AffectsTRelationExtractor::extractWithStartGivenDFS(int curr, int start, shared_ptr<vector<string>> result,
    ...
    shared_ptr<vector<string>> AffectsTRelationExtractor::extractWithEndGiven(int end) {
        shared_ptr<vector<string>> result = make_shared<vector<string>>();
    ...
    void AffectsTRelationExtractor::initAffectsAdjListReversed() {
        affectsAdjListReversedPtr = make_shared<unordered_map<int, unordered_set<int>>>();
        for (int i = 0 ; i < programSize; i++) {
    ...
    void AffectsTRelationExtractor::extractWithEndGivenDFS(int curr, int end, shared_ptr<vector<string>> result,
                                                           shared_ptr<unordered_set<int>> visited) {
        visited->insert(curr);
