...
namespace DE {
		AffectsRelationExtractor::AffectsRelationExtractor(
						...
						: QueryTimeDesignExtractor(std::move(dataModifier), std::move(programNode)) {
			initialize();
		}
		...
		void AffectsRelationExtractor::initialize() {
			...
			modifiesPMap = make_shared<unordered_map<string, shared_ptr<unordered_set<string>>>>(
			...
			for (const shared_ptr<ProcedureNode>& procedure : procedureList) {
				modifiesPMap->insert({procedure->procedureName, getModifiedVariablesFromProcedure(procedure)});
			}
		}

		//! DFS the CFG nodes
		void AffectsRelationExtractor::DFS(int curr,
																			 CFG cfg,
			...
			if (nodeType == AST::ASSIGN_NODE) {
				...
				shared_ptr<AssignNode> assignNode = static_pointer_cast<AssignNode>(nodePtr);
				unordered_set<string> usedVariables = getVariablesFromExprNode(assignNode->expressionNode);
				for (auto variable: usedVariables) {
					if (!lastModifiedMap.count(variable)) continue;
					//! Make pair and insert to affectsTable
					vector<string> affectRelation = {to_string(lastModifiedMap.at(variable)), to_string(curr)};
			...
			if (nodeType == AST::CALL_NODE) {
				...
				shared_ptr<unordered_set<string>> modifiedVars = modifiesPMap->at(callNode->procedureName);
		...
		shared_ptr<ExtractorResult> AffectsRelationExtractor::extractAllRelations() {
			...
			for (const shared_ptr<ProcedureNode>& procedure : procedureList) {
				...
				shared_ptr<vector<int>> visitCount = make_shared<vector<int>>(visitedSize, 0);
				DFS(currLineNumber, currCFG, visitCount, currLineNumber, unordered_map<string, int>());
			}
