...
void UsesSRelationExtractor::save(shared_ptr<ExtractorResult> result) {
  ...
  for (auto entry : *usesSResult->getResult()) {
    dataModifier->saveUsesS(entry);
		...
		UsesSRelationExtractor::extractUsesSHelper(shared_ptr<ASTNode> node, shared_ptr<vector<vector<string>>> result,
																							 shared_ptr<unordered_map<shared_ptr<StmtNode>, int>> stmtNumbers) {
		...
		UsesSRelationExtractor::mapProceduresToUsedVariables(shared_ptr<ProgramNode> rootPtr) {
			shared_ptr<unordered_map<shared_ptr<StmtNode>, int>> stmtNumbers =
		...
		unordered_map<string, unordered_set<string>>
		UsesSRelationExtractor::mapIfAndWhileStmtNoToUsedVariables(shared_ptr<ProgramNode> rootPtr) {
			shared_ptr<unordered_map<shared_ptr<StmtNode>, int>> stmtNumbers =
							ASTUtils::getNodePtrToLineNumMap(rootPtr);
			auto map = unordered_map<string, unordered_set<string>>();

			auto ifAndWhileNodeList =
							extractIfAndWhileNodesFromProcedures(rootPtr);
				...
				while (!queue.empty()) {
					...
					switch (nodeType) {
						...
						case AST::WHILE_NODE: {
							...
							auto variables = getVariablesFromCondExprNode(
											whileNode->condExpr);
							uniqueVarList.insert(variables.begin(), variables.end());
						...
						case AST::ASSIGN_NODE: {
							...
							auto variables = getVariablesFromExprNode(
		...
		void UsesSRelationExtractor::extractCallStmtRelationshipsWithIfAndWhileToOutput(
			...
			auto mappedCallNodesToProcedures =
							extractCallNodesFromProcedures(rootPtr);
		...
		void UsesSRelationExtractor::insertCallsInIfAndWhileForUseS(shared_ptr<ProgramNode> rootPtr,
		...
		void UsesSRelationExtractor::insertCallsForUseS(shared_ptr<ProgramNode> rootPtr, shared_ptr<list<vector<string>>> ans) {
			auto mappedProceduresToUsedVar = mapProceduresToUsedVariables(rootPtr);
			...
			for (auto pair : mappedCallNodesToProcedures) {
				...
				for (auto callNode : listOfCallNodes) {
					...
					extractCallStmtRelationshipsToOutput(
