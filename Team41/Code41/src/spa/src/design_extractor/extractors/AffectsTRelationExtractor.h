...
namespace DE {
    ...
    private:
        ...
        bool extractNoWildcardDFS(int curr, int target, shared_ptr<unordered_set<int>> visited);

        shared_ptr<vector<string>> extractWithStartGiven(int start);
        void extractWithStartGivenDFS(int curr,  int start, shared_ptr<vector<string>> result, shared_ptr<unordered_set<int>> visited);

        shared_ptr<vector<string>> extractWithEndGiven(int end);
        void extractWithEndGivenDFS(int curr,  int end, shared_ptr<vector<string>> result, shared_ptr<unordered_set<int>> visited);
