...
vector<string> AffectsExtractor::extractAffectsWithStartAndEnd(
  ...
  if (!AffectsExtractor::areBothArgsVaild(programNode, startStmtNo,
                                          endStmtNo)) {
    return {};
  }
  ...
  auto endNode = dynamic_pointer_cast<AssignNode>(
      stmtNoToAssignReadAndCallMap.at(to_string(endStmtNo)));
  ...
  unordered_set<string> usedVars =
      EntityExtractor::getVariablesFromExprNode(endNode->expressionNode);
  ...
  AffectsExtractor::extractAffectsWithStartAndEndDFSHelper(
      startStmtNo, endStmtNo, cfg, validPathsList, validPaths, visitedArr);
...
vector<string> AffectsExtractor::extractAffectsWithEndOnly(
  ...
  for (string stmtNo : allStmtNoOfAssignNodes) {
    ...
    vector<string> output =
        AffectsExtractor::extractAffectsWithStartAndEnd(programNode, newArgs);
    if (!output.empty()) {
...
list<vector<string>> AffectsExtractor::extractAllAffectsInProgram(
  ...
  for (auto startStmtNo : allStmtNoOfAssignNodes) {
    for (auto endStmtNo : allStmtNoOfAssignNodes) {
      ...
      vector<string> ans =
          AffectsExtractor::extractAffectsWithStartAndEnd(programNode, args);
      if (!ans.empty()) {
...
void AffectsExtractor::extractAffectsWithStartAndEndDFSHelper(
  ...
  if (!children.empty()) {
    for (int stmtNo : children) {
      if (stmtNo == end) {
        ...
        extractAffectsWithStartAndEndDFSHelper(stmtNo, end, cfg, validPathsList,
                                               validPaths, visitedArr);
      }

      else if (stmtNo <= start) {
        if (!visitedArr.at(stmtNo)) {
          visitedArr.at(stmtNo) = true;
          extractAffectsWithStartAndEndDFSHelper(
              stmtNo, end, cfg, validPathsList, validPaths, visitedArr);
        }
      }
    }
  }
...
vector<string> AffectsExtractor::extractAffectsStarWithStartAndEnd(
  ...
  vector<string> output1 = AffectsExtractor::getIntersectionBetweenAffects(
      programNode, startStmtNo, endStmtNo);
  vector<string> ans;
    ...
    switch (nodeType) {
      ...
      case AST::CALL_NODE: {
        ...
        auto mapProcToModifiedVar =
            ModifiesExtractor::mapProceduresToModifiedVariables(programNode);
...
bool AffectsExtractor::areBothArgsVaild(shared_ptr<ProgramNode> programNode,
                                        int start, int end) {
  shared_ptr<unordered_map<int, shared_ptr<ProcedureNode>>> stmtNoToProcMap =
      ASTUtils::getLineNumToProcMap(programNode);
  ...
  bool startStmtNoIsAssignReadOrCall =
      stmtNoToAssignReadAndCallMap.count(to_string(start)) != 0;
  bool endStmtNoIsAssignReadOrCall =
      stmtNoToAssignReadAndCallMap.count(to_string(end)) != 0;
...
vector<string> AffectsExtractor::getIntersectionBetweenAffects(
  ...
  vector<string> affectsWithStart =
      AffectsExtractor::extractAffectsWithStartOnly(programNode, startArgsOnly);
  vector<string> affectsWithEnd =
      AffectsExtractor::extractAffectsWithEndOnly(programNode, endArgsOnly);
  vector<string> output;
