...
CFG::CFG(ProcedureNode procedureNode,
...
void CFG::processWhileStmtList(int parent,
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (i < size - 1) {
      ...
      // if this is the last statement in the while loop, it should link to its
      // parent
      next = parent;
    }
    if (i > 0 && previousNodeType == AST::IF_NODE) {
...
void CFG::processIfStmtList(int parent, int parentNext,
                            vector<shared_ptr<StmtNode>> stmtList) {
  if (stmtList.empty()) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (i < size - 1) {
      ...
      // if this is the last statement in the if statement, it should link to
      // the statement to be executed after the if statement
      next = parentNext;
    }
    if (i > 0 && previousNodeType == AST::IF_NODE) {
      // do not add next for if statement
