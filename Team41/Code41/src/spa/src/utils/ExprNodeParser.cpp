...
ExprNodeParser::ExprNodeParser(vector<string>& tokens)
    : tokens(tokens), currIdx(ParserConstants::ZERO) {}
...
bool ExprNodeParser::match(const string& s) { return peek() == s; }

int ExprNodeParser::getPriority(const string& s) {
  if (s == ParserConstants::TIMES || s == ParserConstants::DIVIDE ||
      s == ParserConstants::MODULO) {
...
bool ExprNodeParser::buildTree(
    const string& operatorStack,
    const shared_ptr<stack<shared_ptr<ExprNode>>>& nodeStack) {
...
shared_ptr<ExprNode> ExprNodeParser::parse() {
  ...
  shared_ptr<stack<shared_ptr<ExprNode>>> nodeStack =
      make_shared<stack<shared_ptr<ExprNode>>>();
  ...
  while (currIdx < tokens.size()) {
    if (match(ParserConstants::LEFT_BRACKET)) {
      ...
      while (!operatorStack->empty() &&
             operatorStack->top() != ParserConstants::LEFT_BRACKET) {
      ...
      // is operator
      while (!operatorStack->empty() &&
             getPriority(operatorStack->top()) >= getPriority(peek())) {
        buildTree(operatorStack->top(), nodeStack);
        operatorStack->pop();
      }
      operatorStack->push(pop());
