...
class CacheManager {
 public:
  ...
  shared_ptr<Table> getNextTStatements(shared_ptr<Cachable> nextTTable,
                                       int stmtNo);
  shared_ptr<Table> getPreviousTStatements(shared_ptr<Cachable> nextTTable,
                                           int stmtNo);
  shared_ptr<Table> getNextTResult(shared_ptr<Cachable> nextTTable,
                                   int precedingStatement,
                                   int ensuingStatement);
  shared_ptr<Table> getAffectsTable(shared_ptr<Cachable> affectsTable);
  shared_ptr<Table> getAffectedStatements(shared_ptr<Cachable> affectsTable,
                                          int stmtNo);
  shared_ptr<Table> getAffectingStatements(shared_ptr<Cachable> affectsTable,
                                           int stmtNo);
  shared_ptr<Table> getAffectsResult(shared_ptr<Cachable> affectsTable,
                                     int affectingStatement,
                                     int affectedStatement);
  shared_ptr<Table> getAffectsTTable(shared_ptr<Cachable> affectsTTable);
  shared_ptr<Table> getAffectedTStatements(shared_ptr<Cachable> affectsTTable,
                                           int stmtNo);
  shared_ptr<Table> getAffectingTStatements(shared_ptr<Cachable> affectsTTable,
                                            int stmtNo);
  shared_ptr<Table> getAffectsTResult(shared_ptr<Cachable> affectsTTable,
                                      int affectingStatement,
                                      int affectedStatement);
 ...
 private:
  ...
  shared_ptr<Table> getExactRelationHelper(
