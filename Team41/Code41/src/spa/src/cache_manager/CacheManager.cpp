...
shared_ptr<Table> CacheManager::getStatementsHelper(
    ...
    if (cachable->areAllRelationsCached()) {
        ...
        if (range.empty()) {
            ...
            CacheContentLoader cacheContentLoader = CacheContentLoader(this->designExtractorRetriever);
...
shared_ptr<Table> CacheManager::getNextTStatements(shared_ptr<Cachable> nextTTable, int stmtNo) {
    int columnNumber = 0;
    return getStatementsHelper(nextTTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getNextTStatements);
}

shared_ptr<Table> CacheManager::getPreviousTStatements(shared_ptr<Cachable> nextTTable, int stmtNo) {
    int columnNumber = 1;
    return getStatementsHelper(nextTTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getPreviousTStatements);
}

shared_ptr<Table> CacheManager::getNextTResult(shared_ptr<Cachable> nextTTable, int precedingStatement, int ensuingStatement) {
    return getExactRelationHelper(nextTTable, vector<int>{precedingStatement, ensuingStatement}, &CacheContentLoader::getNextTResult);
}

shared_ptr<Table> CacheManager::getAffectsTable(shared_ptr<Cachable> affectsTable) {
    getAllRelationsHelper(affectsTable, &CacheContentLoader::getAffectsRelations);
    return affectsTable;
}

shared_ptr<Table> CacheManager::getAffectedStatements(shared_ptr<Cachable> affectsTable, int stmtNo) {
    int columnNumber = 0;
    return getStatementsHelper(affectsTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getAffectedStatements);
}

shared_ptr<Table> CacheManager::getAffectingStatements(shared_ptr<Cachable> affectsTable, int stmtNo) {
    int columnNumber = 1;
    return getStatementsHelper(affectsTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getAffectingStatements);
}

shared_ptr<Table> CacheManager::getAffectsResult(shared_ptr<Cachable> affectsTable, int affectingStatement, int affectedStatement) {
    return getExactRelationHelper(affectsTable, vector<int>{affectingStatement, affectedStatement}, &CacheContentLoader::getAffectsResult);
}

shared_ptr<Table> CacheManager::getAffectsTTable(shared_ptr<Cachable> affectsTTable) {
    getAllRelationsHelper(affectsTTable, &CacheContentLoader::getAffectsTRelations);
    return affectsTTable;
}

shared_ptr<Table> CacheManager::getAffectedTStatements(shared_ptr<Cachable> affectsTTable, int stmtNo) {
    int columnNumber = 0;
    return getStatementsHelper(affectsTTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getAffectedTStatements);
}

shared_ptr<Table> CacheManager::getAffectingTStatements(shared_ptr<Cachable> affectsTTable, int stmtNo) {
    int columnNumber = 1;
    return getStatementsHelper(affectsTTable, vector<int>{stmtNo, columnNumber}, &CacheContentLoader::getAffectingTStatements);
}

shared_ptr<Table> CacheManager::getAffectsTResult(shared_ptr<Cachable> affectsTTable, int affectingStatement, int affectedStatement) {
    return getExactRelationHelper(affectsTTable, vector<int>{affectingStatement, affectedStatement}, &CacheContentLoader::getAffectsTResult);
}